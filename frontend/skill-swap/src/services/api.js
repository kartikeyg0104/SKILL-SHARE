...
class ApiService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          // For social features, return empty data instead of throwing error
          if (endpoint.includes('/social/') && (response.status === 403 || response.status === 401)) {
            console.warn('Social API call failed due to authentication. Using fallback data.');
            return this.getFallbackSocialData(endpoint);
          }
        ...
        if (response.status === 400 && data.details && Array.isArray(data.details)) {
          const validationMessages = data.details.map(detail => detail.msg).join(', ');
  ...
  // Fallback data for social features when authentication fails
  getFallbackSocialData(endpoint) {
    if (endpoint.includes('/trending')) {
      return [
        { tag: 'JavaScript', count: 156 },
        { tag: 'React', count: 89 },
        { tag: 'Python', count: 67 },
        { tag: 'WebDevelopment', count: 45 },
        { tag: 'DataScience', count: 34 }
      ];
    }
    
    if (endpoint.includes('/suggested-users')) {
      return [];
    }
    
    if (endpoint.includes('/posts/public')) {
